...
TEST(diccionario, dicc_string_bool) {
	...
	ASSERT_FALSE(d.def("fsd1"));
	ASSERT_FALSE(d.def("f23j"));
	ASSERT_FALSE(d.def("td3e"));
	d.definir("false", false);
	ASSERT_TRUE(d.def("false"));
	ASSERT_FALSE(d.def("iajd"));
	ASSERT_FALSE(d.def("aisjdoi"));
	ASSERT_FALSE(d.obtener("false"));
	d.definir("true", true);
	ASSERT_TRUE(d.def("true"));
	ASSERT_TRUE(d.def("false"));
	ASSERT_FALSE(d.def("iojasdoh"));
	ASSERT_FALSE(d.obtener("false"));
	ASSERT_TRUE(d.obtener("true"));
	d.definir("true", false);
	ASSERT_TRUE(d.def("true"));
	ASSERT_TRUE(d.def("false"));
	ASSERT_FALSE(d.def("ohdaosd"));
	ASSERT_FALSE(d.obtener("true"));
	ASSERT_FALSE(d.obtener("false"));	
}
